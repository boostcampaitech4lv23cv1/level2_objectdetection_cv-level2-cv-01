    ...
    def __init__(self, opt, name, run_id, data_dict, job_type='Training'):
        ...
        if isinstance(opt.resume, str):  # checks resume from artifact
            if opt.resume.startswith(WANDB_ARTIFACT_PREFIX):
                ...
                self.wandb_run = wandb.init(entity='bc_cv01', id=run_id, project=project, resume='allow')
                opt.resume = model_artifact_name
        elif self.wandb:
            self.wandb_run = wandb.init(config=opt,
                                        entity='bc_cv01',
                                        resume="allow",
                                        project='bc_lv2-object_detection' if opt.project == 'runs/train' else Path(opt.project).stem,
                                        name=name,
                                        job_type=job_type,
